...
void mergeParSub_preComp_toyIntegrals_MC(int q2Bin, int parity, int tagFlag, int maxSeed, int minSeed=1, int year=2016, int toy=0, int vers=-1)
{
  ...
  string efffilename = Form((parity==0?"files/KDEeff_b%i_ev_%i_toy%i_v%i.root":"files/KDEeff_b%i_od_%i_toy%i_v%i.root"),q2Bin,year,toy,vers);
  ...
  for (int seed=minSeed; seed<=maxSeed; ++seed) {
    ...
    filename = Form("tmpint_v%i/PreIntMC_",vers)+shortString+Form("_s%i_%i_toy%i.root",seed,year,toy);
