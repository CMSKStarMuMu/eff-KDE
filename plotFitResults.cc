...
void plotFitResultsBin(int parity, int ParIndx, bool plotCT, bool plotWT, bool plotRECO)
{
  ...
  TFile* finGen = TFile::Open("fitResultWeighted_genMC.root");
  if ( !finGen || finGen->IsZombie() ) {
    cout<<"Missing gen file: fitResultWeighted_genMC.root"<<endl;
    return;
  }
  TFile* finReco;
  TFile* finFullReco;
  if (plotCT || plotWT) finReco = TFile::Open("fitResultWeighted_recoMC_singleComponent.root");
  if (plotRECO) finFullReco = TFile::Open("fitResultWeighted_recoMC_fullAngular.root");
  ...
  string confString = "fitResultWeighted_";
