...
void plotEff_rooKeysBin(int q2Bin, bool tagFlag, int xbins, int ybins, int zbins, float width);

TCanvas* c  [2*nBins];
TCanvas* c1 [2*nBins];
TCanvas* c3 [2*nBins];

void plotEff_rooKeys(int q2Bin, int tagFlag, int xbins=25, int ybins = 0, int zbins = 0, float width = 1)
{
  ...
  if ( q2Bin > -1 && q2Bin < nBins ) {
    if (tagFlag < 2 && tagFlag > -1) {
      ...
      plotEff_rooKeysBin(q2Bin, (tagFlag==1), xbins, ybins, zbins, width);
    }
    if (tagFlag == 2) {
      cout<<"Plotting efficiency for q2 bin "<<q2Bin<<" correctly and wrongly tagged events"<<endl;
      plotEff_rooKeysBin(q2Bin, true,  xbins, ybins, zbins, width);
      plotEff_rooKeysBin(q2Bin, false, xbins, ybins, zbins, width);
    }
  }
  if (q2Bin == -1) {
