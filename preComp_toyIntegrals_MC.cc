...
void preComp_Integrals_MCBin(int q2Bin, int parity, int tagFlag, unsigned cnt_hit, int seed, int year, int toy, int vers)
{
  ...
  string filename = fileDir + Form((parity==0?"/files/KDEeff_b%i_ev_%i_toy%i_v%i.root":"/files/KDEeff_b%i_od_%i_toy%i_v%i.root"),q2Bin,year,toy,vers);
  ...
  TFile* fout = TFile::Open(Form("%s/tmpint_v%i/PreIntMC_%s_s%i_%i_toy%i.root",fileDir.c_str(),vers,shortString.c_str(),seed,year,toy),"RECREATE");
  fout->cd();
  hvolume ->Write();
  hcnt_p  ->Write();
  hcnt_m  ->Write();
...
void preComp_Integrals_MCBin2(int q2Bin, int parity, int tagFlag, unsigned cnt_hit, int seed, int year, int toy, int vers)
{
  if ( tagFlag==-1 )
    for (tagFlag=0; tagFlag<2; ++tagFlag)
      preComp_Integrals_MCBin(q2Bin, parity, tagFlag, cnt_hit, seed, year, toy, vers);
  else
    preComp_Integrals_MCBin(q2Bin, parity, tagFlag, cnt_hit, seed, year, toy, vers);
}

void preComp_Integrals_MCBin1(int q2Bin, int parity, int tagFlag, unsigned cnt_hit, int seed, int year, int toy, int vers)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      preComp_Integrals_MCBin2(q2Bin, parity, tagFlag, cnt_hit, seed, year, toy, vers);
  else
    preComp_Integrals_MCBin2(q2Bin, parity, tagFlag, cnt_hit, seed, year, toy, vers);
}
...
int main(int argc, char** argv)
{
  ...
  if ( argc >= 8 ) toy     = atoi(argv[7]);
  if ( argc >= 9 ) vers    = atoi(argv[8]);
  ...
  if ( q2Bin==-1 )
    for (q2Bin=0; q2Bin<nBins; ++q2Bin)
      preComp_Integrals_MCBin1(q2Bin, parity, tagFlag, cnt_hit, seed, year, toy, vers);
  else
    preComp_Integrals_MCBin1(q2Bin, parity, tagFlag, cnt_hit, seed, year, toy, vers);
