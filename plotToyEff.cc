...
void plotToyEffBin(int q2Bin, int parity, bool doClosure, int year)
{
  string shortString = Form("b%ip%i",q2Bin,parity);
  cout<<"Conf: "<<shortString<<endl;

  string confString = "plotToyEff_d/effKDE_test_"+shortString;
  gStyle->SetOptStat(0);
  ...
  // import KDE efficiency toy histograms
  string effDir = "/eos/user/a/aboletti/BdToKstarMuMu/eff-KDE-Swave/files/";
  string filenameToy [nToys];
  TFile* finToy [nToys];
  TH3D* effCHistToy [nToys];
  TH3D* effWHistToy [nToys];
  RooDataHist* effCDataToy [nToys];
  RooDataHist* effWDataToy [nToys];
  RooAbsReal* effCToy [nToys];
  RooAbsReal* effWToy [nToys];
  for (int iToy=0; iToy<nToys; ++iToy) {
    filenameToy[iToy] = effDir + Form((parity==0?"KDEeff_b%i_ev_%i_toy%i.root":"KDEeff_b%i_od_%i_toy%i.root"),q2Bin, year, iToy);
    finToy[iToy] = new TFile( filenameToy[iToy].c_str(), "READ" );
    if ( !finToy[iToy] || !finToy[iToy]->IsOpen() ) {
      cout<<"File not found: "<<filenameToy[iToy]<<endl;
      return;
    }
    effCHistToy[iToy] = (TH3D*)finToy[iToy]->Get(("effCHist_"+shortString).c_str());
    effWHistToy[iToy] = (TH3D*)finToy[iToy]->Get(("effWHist_"+shortString).c_str());
    if ( !effCHistToy[iToy] || effCHistToy[iToy]->IsZombie() ) {
      cout<<"Correct-tag efficiency histograms not found in file: "<<filenameToy[iToy]<<endl;
      doCT = false;
    }
    ...
    if (doCT) {
      effCDataToy[iToy] = new RooDataHist(Form("effCDataToy%i",iToy),Form("effCDataToy%i",iToy),vars,effCHistToy[iToy]);
      effCToy[iToy] = new RooHistFunc(Form("effCToy%i",iToy),Form("effCToy%i",iToy),vars,*effCDataToy[iToy],1);
    }
    if (doWT) {
      effWDataToy[iToy] = new RooDataHist(Form("effWDataToy%i",iToy),Form("effWDataToy%i",iToy),vars,effWHistToy[iToy]);
      effWToy[iToy] = new RooHistFunc(Form("effWToy%i",iToy),Form("effWToy%i",iToy),vars,*effWDataToy[iToy],1);
    }
  }

    
  // import KDE efficiency histograms
  string filename = effDir + Form((parity==0?"KDEeff_b%i_ev_%i.root":"KDEeff_b%i_od_%i.root"),q2Bin, year);
  ...
  for (int i=0; i<5; ++i) for (int j=0; j<5; ++j) {
	...
	effWDist[2]->SetTitle( Form("Mistag efficiency (q2-bin %i, %s) slice ctK=%1.2f ctL=%1.2f;#phi;Efficiency",q2Bin,(parity==0?"even":"odd"),centA,centB) );
	  ...
	  for (int iToy=0; iToy<nToys; ++iToy)
	    effCToy[iToy]->plotOn( fsC[ivar].back(), LineColor(kBlue), LineWidth(2), Name(Form("effC%s_%i_%i_%s_toy%i",varCoord[ivar],i,j,shortString.c_str(),iToy))) ;
	  ...
	  for (int iToy=0; iToy<nToys; ++iToy)
	    effWToy[iToy]->plotOn( fsW[ivar].back(), LineColor(kBlue), LineWidth(2), Name(Form("effW%s_%i_%i_%s_toy%i",varCoord[ivar],i,j,shortString.c_str(),iToy))) ;
	  ...
	  effCSlice_corr[ivar].back()->GetYaxis()->SetTitleOffset(1.7);
          effCSlice_corr[ivar].back()->Draw();
	  ...
	  gPad->SetLeftMargin(0.18);
	  effWSlice_corr[ivar].back()->GetYaxis()->SetTitleOffset(1.7);
          effWSlice_corr[ivar].back()->Draw();
  ...
  for (int ivar=0; ivar< 3; ivar++){
    if (doCT) {
      for (vector<TH1D*>::iterator hist = effCSlice_corr[ivar].begin(); hist != effCSlice_corr[ivar].end(); ++hist) 
        (*hist)->SetMaximum(maxEffC[ivar]*1.1);
      csC[ivar][confIndex]->SaveAs( (confString+Form("_toyeff-ct_%sslices_comp_dp%i_%i.pdf",varNames[ivar] ,(int)(border*200), year)).c_str() );
    }
    if (doWT) {
      for (vector<TH1D*>::iterator hist = effWSlice_corr[ivar].begin(); hist != effWSlice_corr[ivar].end(); ++hist) 
        (*hist)->SetMaximum(maxEffW[ivar]*1.1);
      csW[ivar][confIndex]->SaveAs( (confString+Form("_toyeff-wt_%sslices_comp_dp%i_%i.pdf",varNames[ivar] ,(int)(border*200), year)).c_str() );
    }
  }
