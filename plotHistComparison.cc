...
void plotHistBin(int q2Bin, int effIndx, int parity, int year, int vers)
{
  ...
  case 4: longString = "Selected mistagged"  +longString; break;
  ...
  string filename = Form("effDataset_b%i_%i%s.root",q2Bin,year,XGBstr.c_str());
  if (q2Bin==4 && effIndx==2)
    filename = Form("effDataset_b%i_%i%s_den.root",q2Bin,year,XGBstr.c_str());
  ...
  RooWorkspace* wsp_data2= (RooWorkspace*)fin_data->Get(Form("ws2_b%ip%i",q2Bin,parity));
  ...
  if (effIndx==2 && wsp_data2) {
    data->append(*((RooDataSet*)wsp_data2->data(Form((parity==0?"data_den2_ev_b%i":"data_den2_od_b%i"),q2Bin))));
  }
  auto den2ds = (RooDataSet*)wsp_data->data(Form((parity==0?"data_den2_ev_b%i":"data_den2_od_b%i"),q2Bin));
  if (effIndx==2 && den2ds) data->append(*den2ds);
  if ( effIndx==5 ) {
  ...
  delete csx[confIndex];
  delete csy[confIndex];
  delete csz[confIndex];
  for (int i=0; i<25; ++i) {
    for (int iHist=0; iHist<histSliceZ[i].size(); ++iHist) {
      delete (histSliceX[i])[iHist];
      delete (histSliceY[i])[iHist];
      delete (histSliceZ[i])[iHist];
    }
    delete distSliceX[i];
    delete distSliceY[i];
    delete distSliceZ[i];
  ...
  delete cp1[confIndex];
  for (int iHist=0; iHist<histProj1Z.size(); ++iHist) {
    delete histProj1X[iHist];
    delete histProj1Y[iHist];
    delete histProj1Z[iHist];
  }
  delete distProj1X;
  delete distProj1Y;
  delete distProj1Z;
